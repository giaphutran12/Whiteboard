        ...
        if($_POST["actionContact"] == "insertContact")
        {
            ...
            if(!mysqli_stmt_prepare($stmt, $sql)){
                ...
                if($rowCount > 0){
                    ...
                    //attempted to check for correct format, but failed
                    // else if (preg_match('/^\d{4}$/', $contactLocal) && preg_match('/^\d{3}-\d{3}-\d{4}$/', $contactCell)) {
                    //         mysqli_stmt_bind_param($stmt, "sss", $contactName, $contactLocal, $contactCell);
                    //         mysqli_stmt_execute($stmt);
                    //         echo '<p>Contact inserted!</p>';
                    //     } 
                    //     else {
                    //         echo '<p>Error: Invalid format. $contactLocal should be 4 digits, and $contactCell should be in the format ddd-ddd-dddd.</p>';
